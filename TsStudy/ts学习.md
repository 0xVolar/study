...
- **只读数组**
  ...
  - **只读数组不能与泛型写到一起**，但是可以使用**ReadonlyArry<type>和Readonly<type[]>**这两个专门的泛型来生成只读数组类型。前一个是数组中成员是类型，后一个中成员的类型是数组。
- **多维数组**：可以使用`type[][]`这个方法来声明多维数组，数组中的成员类型为所定义的type

## 元组

- 元组的是ts独有的类型，**将类型名写在方括号中**中即可表示为元组。**数组类型写在括号外，元组写在括号内**
- **元组必须显示的给出类型声明**，否则ts会将其推断为数组。由此**元组的长度是有限的，并不是动态变化的**
- **在类型后面加？号表示该类型是可选的，即可被忽略**，并且可选类型必须声明在必填类型的后面
- `...`,这是拓展运算符，当在元组中使用这个符号之后就表示改元组长度不限，eg：
  - `type A = [string, ...number[]]`
  - 这段代码表示该元组第一个类型为string，后面的是一个number的数组，这就完成了元组的长度不限，因为数组长度不限
  - **在拓展符后面必须跟数组或者元组**，并且拓展符在元组中的使用位置是没有要求的。
- **只读元组**
  - `type a = readonly [number, number]` || `type = Readonly<[number, number]>`
  - 只读元组是元组的父类型，应用规则和数组的一样。

## symbol
- symbol是新引入的类型，由Symbol()函数进行生成，每一个生成的symbol都是独一无二的
- **unique symbol**
  - unique symbol是symbol的子类型，在使用时和数组的父子之间用法相同
  - 这是symbol的字类型，用来表示某个具体的symbol的值。**只能用const进行声明，并且不能进行修改**
  - `const x:unique symbol = Symbol();`在使用const进行声明时，unique可以进行省略，默认声明的是unique
  - **每一个声明的unique symbol**虽然类型相同，但是他们表示的值是不相同的。他们是属于两个不同的值类型

## Function

- **在写函数时，参数必须要有参数名，否则ts会讲起推断为any**
- ts允许传入的参数少于定义的参数数量，但是不允许传入的参数的数量大于定义的参数数量。
- **FUNCTION类型**：ts提供FUNCTION类型表示函数类型，当为变量声明该类型时，不限参数输入数量，不限参数输入类型