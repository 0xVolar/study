...
# 事件 #
- 在solidity中**事件也是一个可以被继承的对象**，并且事件是保存在链上的。并且可以提供证明事件的合约，通过提供区块头进行查询。
## 事件的声明 ##
...
- `event IndexedLog(事件的类型1 indexed 变量1, uint val);`
    ...
    - 带有索引的变量可以在链外进行搜索查询。
- 通过**indexed**进行标记的数据，会被保存到一个新的数据结构叫做**topic**，在其中不会保存所有的event的数据，只会保存想要的数据。并且**topic中的容量为32bytes**，对于**应用类型的值，保存的是其经过keccak256之后的值**。对于**其他的没有经过indexed标记的event会被进行ABI编码之后保存在整个数据中log的部分**。

## 匿名事件（anonymous）
- 在匿名事件的存在，表示**在进行过滤查找时不能通过事件的名称进行查找，只能通过合约地址进行过滤**。并且在**匿名事件中可以使用4个indexed**。同时也可以通过匿名事件来伪装成其他事件。
...
- 对于在继承时，只会**创建一个合约在链上**，而在进行**编码的时候会将继承的合约的内容一起编码到新的合约**当中。所以在**子合约中调用父合约中的方法的，其实是属于消息的内部调用，而非一个消息调用**。在子合约中不能声明与父合约中同名的变量，这是一个错误。

## 多线继承 ##
- 多线继承的规则：**从基类到派生**的顺序关系，即为从简单到复杂。
- 如果合约C中的方法在多个合约都有的话，在方法重写声明的时候在override后面要用括号将继承的合约加入
    - `function foo() public pure override(X, Y) returns (string memory) {}`
    - 在override中的合约顺序，**不用遵守多线继承的规则**

- 如果在**子合约中想对父合约中的方法进行修改的话，使用override进行修饰，而且父合约中的相应方法必须要使用virtual进行修饰**，对于private方法不能使用virtual修饰
- **如果不想函数被覆盖的话可以不使用virtual标记，但是对于定义在interface中的函数，会自动地默认函数全部都加了virtual函数**，从0.0.80版本开始，当方法没有被多个合约进行覆盖的话，可以不使用override关键词。
- 如果**在父合约中实现了getter方法的话，在子合约中可以使用状态变量来进行override**，同时如果状态变量实现了对方法的override，但是并不能对状态变量进行override。
...
## salt ##
- 在使用**new()**方法进行构造新合约时，可以加入**salt**，类型为**bytes32**，通过使用**salt**可以进行合约的地址预测
  - 预测过程：**当前工厂合约的地址** + **salt** + **被部署合约的机器码bytecode**，bytecode为字节码加上调用的合约地址
  - 在之前合约没被销毁之前，**每一个salt在相同工厂合约中只能用一次**
...
- ` selfdestruct(address) `
  - 使用**selfdestruct**方法可以销毁合约，但是必须在方法里面传入一个地址参数，即为，**将要销毁的合约剩余的主币强制发送到传入的地址**，不论该地址是否有接收主币**fallback**函数。
  - 可以使用自毁的方法强制发送主币
...
- `keccak256(abi.encodePacked(text, num, add));`
  ...
  - **abi.encodePacked**，返回的是一串**16进制的数字**，并不会自动补零,这样就可能存在隐患，即**不同的输入产生相同的结果**，可能会造成哈希碰撞。

# ABI的解码 #
- 函数的执行，会将函数名，传入参数，编程一段ABI的编码，使用**abi.decode**的方法可以将abi编码后的数据进行还原，**前提是知道这些编码所对应的数据的类型**